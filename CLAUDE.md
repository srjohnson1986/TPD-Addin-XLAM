...
One custom ribbon tab ("TPD") with two groups — **EQ List Tools** (EQ Count + the picker/custom paths) and **Defaults** (the one-click "EQ List" / "Schedule" / "Split Sheets" buttons that run straight from saved defaults, beside "Set TPD Defaults"; "Schedule" is greyed out until a schedule-automation flow exists) — defined in `customUI/customUI14.xml`. Ribbon buttons call thin wrapper subs in `modRibbonCallbacks`, which own the global `gRibbon` reference and `RibbonOnLoad` (triggers `modStartup.InitializeAddIn`, which stamps the running version into the registry-backed preference store; the `_Resources` sheet + embedded logo come from the base `.xlam`).
...
- **Schedule** — no modules yet. "Default Schedule Header" was removed in #96; the header builder `InsertDefaultCustScheduleHeader` lives in `modHelpers_SheetSetup` awaiting a "Create Customer Schedule" flow (copy source → new sheet before formatting, like EQ List).
...
- **Set TPD Defaults dialog ([#25](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/25))** — **mostly delivered.** `frmSetTPDDefaults` is a working modal (ribbon: TPD → Defaults → Set TPD Defaults) with EQ List / Schedule / Split Sheets / Logo tabs; comma-separated MultiLine TextBox per tab, normalized on paste + exit; per-tab Restore defaults; OK → `SaveAllDefaults` + status-bar confirm; Cancel/Esc/X discard. **#96 done for EQ List + Split Sheets:** one-click ribbon buttons run each flow from the `DefaultUser*` value (→ shipped default); the pickers now write separate `LastUsed*` keys and resolve `LastUsed*` → `DefaultUser*` → shipped (three-layer model — this replaced the old single-key model). **#98 done:** pickers dropped their inline default arrays. Open follow-ups: **Customer Schedule automation flow** + enabling the greyed-out "Schedule" one-click button; user-settable logo ([#95](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/95)/[#109](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/109)); EQ List behaviour toggles ([#97](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/97)); in-page Generate buttons ([#100](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/100)); the `txtImgPath` / `cmdBrowse` vestige on `CustEQListColumnPickerForm` from [#82](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/82).
