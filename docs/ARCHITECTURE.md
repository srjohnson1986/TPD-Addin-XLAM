...
The add-in exposes one custom ribbon tab ("TPD") with three groups: **EQ List Tools**, **Schedule Tools**, and **Settings** (the single "Set TPD Defaults" button), defined in `customUI/customUI14.xml`. Ribbon buttons call thin wrapper subs in `modRibbonCallbacks`, which delegate to the feature modules described below. Startup (`modStartup.InitializeAddIn`) runs from `RibbonOnLoad` and stamps the running version into the registry-backed preference store. Saved settings live in the Windows registry per user (`modPreferences`), not in the workbook. The hidden `_Resources` sheet and its embedded default-logo shape come from the base `.xlam`, not from code.
...
| Module | Purpose |
|---|---|
| `modMain_CustEQList` | "Create Customer EQ List" entry point. Shows `CustEQListColumnPickerForm`, copies all rows to a new sheet, deletes unselected columns, formats the sheet, inserts the EQ header block + the embedded default logo (`InsertDefaultLogo … "right-top"`, same call as the "Default EQ List Header" command — [#82](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/82)), and freezes panes. |
...
| `CustEQListColumnPickerForm` | UserForm for choosing which columns to keep when creating a Customer EQ List. `LoadColumns` builds the checkbox grid (`modHelpers_CheckboxLayout`) then applies the saved-or-default selection (`ApplyColumnSelection`, shared with the split picker — [#81](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/81)); OK saves the selection after checking at least one column is ticked (`HasColumnSelection`, matching the split picker — [#104](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/104)). The `txtImgPath` / `cmdBrowse` controls are vestigial after [#82](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/82) (no handlers) pending a VBE cleanup or #25. Slated for removal in the one-click EQ List refactor ([#25](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/25)). |
...
| Module | Purpose |
|---|---|
| `modPreferences` | Per-user key/value preference storage in the Windows registry via `SaveSetting`/`GetSetting` (`HKCU\Software\VB and VBA Program Settings\TPD_Addin\Preferences`). Registry rather than a sheet inside the `.xlam` because Excel never saves an add-in on exit, so in-workbook writes were silently lost on quit ([#84](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/84)). `SavePref`/`LoadPref`/`DeletePref` plus `SaveColumnList`/`LoadColumnList` for comma-separated column lists; a missing key returns the caller's default. Generic multi-key helpers: `SavePrefs` (write an array of key/value pairs, returns `False` if any write failed, no rollback) and `AllPrefsUnset` (True iff every listed key reads back empty). Also the Set TPD Defaults dialog support: `NormalizeColumnList` (tabs/CR/LF → `, `, drop empties, keep dupes), `CollapseWhitespace` (a header name's compare form — tabs → spaces, then delegates to `modHelpers_Strings.NormalizeCellText`), `SaveAllDefaults` and `DefaultsNeverSaved` — thin callers of `SavePrefs`/`AllPrefsUnset` over the one shared list of the four dialog keys ([#102](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/102)). |
| `modPreferences_KeyMap` | Central list of every preference key so keys never float as loose string literals. The four column keys — `PREF_EQLIST_COLUMNS` (`DefaultUserEqListColumns`), `PREF_SCHEDULE_COLUMNS` (`DefaultUserScheduleColumns`), `PREF_SPLIT_COLUMNS` (`DefaultUserSplitSheetsColumns`), `PREF_SPLIT_GROUPCOL` (`DefaultUserSplitSheetsGroupColumn`) — are one shared set: the Set TPD Defaults dialog writes them, each picker pre-fills from them and writes the user's per-run choice back. Plus `PREF_VERSION`. |
| `modPreferences_Defaults` | The hard-coded fallback column lists (`DefaultEqListColumns`, `DefaultScheduleColumns`, `DefaultSplitColumns`, `DefaultSplitGroupColumn`), comma-separated. Seed the dialog's first run, back its per-page "Restore defaults", and are the value `LoadPref` returns for an unsaved key. One home replacing the arrays previously inline in each picker. |
...
- **Set TPD Defaults dialog ([#25](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/25))** — **partially delivered.** The dialog itself (`frmSetTPDDefaults` + `modMain_SetTPDDefaults` + the `modPreferences` / `modPreferences_Defaults` support) is built and wired to the ribbon; it writes the shared `DefaultUser*` column keys the pickers pre-fill from. Still open as follow-ups: user-settable default logo (Browse → embed into `_Resources`); headless one-click feature buttons; wiring the EQ List behaviour toggles Steve sketched on the form (`cbxRemoveParentRows` etc.); repointing the two pickers' inline default arrays onto `modPreferences_Defaults`. `CustEQListColumnPickerForm` stays (it's the in-the-moment override) — the `txtImgPath` / `cmdBrowse` vestige from [#82](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/82) is still there to clean up.
