...
| `modHelpers_Image` | `SafeInsertLogoAtRight` (error-wrapped logo-from-file insertion, called by the Customer EQ List flow — usually with an empty path now) and `PastePicture` (clipboard image → `StdPicture`, used by `frmSetTPDDefaults` for the logo preview). Both are legacy of the file-path logo approach; slated for removal with the one-click EQ List refactor ([#25](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/25)). |
