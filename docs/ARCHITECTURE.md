...
| `frmSetTPDDefaults` | The "Set TPD Defaults" dialog ([#25](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/25), [#94](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/94)). MultiPage (`mpgPages`) with EQ List / Schedule / Split Sheets / Logo tabs. Each column tab is a MultiLine TextBox of plain comma-separated header names, normalized on paste (Ctrl+V caught on `KeyUp`) and on `Exit`; per-tab "Restore defaults" resets from `modPreferences_Defaults`. OK → `SaveAllDefaults` → status-bar confirm (no MsgBox on success); a write failure keeps the form open with a `vbCritical` box. Cancel / Esc / title-bar X all discard silently. The Logo tab previews the embedded `_Resources` `DefaultLogo` shape via `PastePicture` — read-only; user-settable logo is deferred. `lblVersion` shows `ADDIN_VERSION` (#94). |
