...
| `modHelpers_Logo` | Positions and resizes the embedded default logo: `InsertDefaultLogo` (horizontal/vertical alignment keywords, optional scale-to-max-rows) and `ResizeImageToMaxRows`. Also `PastePicture` (clipboard image → `StdPicture`, used by `frmSetTPDDefaults` for the logo preview). The file-path `InsertLogoAtRight` / `SafeInsertLogoAtRight` pair was removed in [#82](https://github.com/srjohnson1986/TPD-Addin-XLAM/issues/82) — the Customer EQ List flow now uses `InsertDefaultLogo` like every other logo caller. |
